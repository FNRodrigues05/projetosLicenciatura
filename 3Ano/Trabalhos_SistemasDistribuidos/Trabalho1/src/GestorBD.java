...
public class GestorBD {
    ...
    public void devolverEmprestimo(int idEmprestimo) throws Exception {
        ...
        try {
            ...
            // Atualizar emprestimo para DEVOLVIDO
            ...
            // atualizar num_copias para LIVRO
            // e ajustar estado do recurso após devolução
            String tipoRecurso;
            ...
            if ("LIVRO".equals(tipoRecurso)) {
                // Verifica disponibilidade pela contagem de
                // emprestimos ATIVOS.
                int numCopias;
                ...
                try (PreparedStatement ps = con.prepareStatement(
                    ...
                    try (ResultSet rs = ps.executeQuery()) {
                        rs.next();
                        ativos = rs.getInt(1);// número de emprestimos ATIVOS
                    }
                }
                // Se agora há menos ativos que cópias, fica DISPONIVEL senão mantem
                // INDISPONIVEL

                // atualiza estado do recurso
            ...
            } else if ("COMPUTADOR".equals(tipoRecurso)) {// se for computador
                ...
                // se não é livro nem computador sala de estudo
                try (PreparedStatement ps = con.prepareStatement(
                        "UPDATE recurso SET estado = 'DISPONIVEL'::estado_recurso_enum WHERE id_recurso = ?")) {
                    ps.setInt(1, idRecurso);
                    ps.executeUpdate();
    ...
    public java.util.List<String> listarRecursosPorEstadoAdmin(String estadoAdmin) throws Exception {
        ...
        try (Connection con = getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                while (rs.next()) {// header quando para cada tipo de recurso
                    ...
                    // Agora é adicionado o recurso em si
                    res.add(
    ...
    public List<String> listarRecursosFiltrado(String tipo, String estadoOp, String estadoAdmin, String texto)
        ...
        if (tipo == null || tipo.isEmpty() || tipo.equalsIgnoreCase("LIVRO")) {// listar livros
            out.addAll(listarLivrosFiltrado(estadoOp, estadoAdmin, texto));
        }
        if (tipo == null || tipo.isEmpty() || tipo.equalsIgnoreCase("COMPUTADOR")) {// listar computadores
            out.addAll(listarComputadoresFiltrado(estadoOp, estadoAdmin, texto));
        }
        if (tipo == null || tipo.isEmpty() || tipo.equalsIgnoreCase("SALA")) {// listar salas
    ...
    private List<String> listarLivrosFiltrado(String estadoOp, String estadoAdmin, String texto) throws Exception {
        ...
        if (estadoOp != null) {// se foi fornecido filtro por estado operacional
            sql.append(" AND r.estado = ?::estado_recurso_enum ");
            ps.add(estadoOp);
        }
        if (estadoAdmin != null) {// se foi fornecido filtro por estado admin
            sql.append(" AND r.estado_admin = ?::estado_admin_enum ");
            ps.add(estadoAdmin);
        }
        if (texto != null) {// se foi fornecido filtro por texto
        ...
        try (Connection con = getConnection(); PreparedStatement st = con.prepareStatement(sql.toString())) {
            ...
            try (ResultSet rs = st.executeQuery()) {
                ...
                while (rs.next()) {// "tabela" do output
    ...
    public void alterarUtilizacoesComputador(int idRecurso, int novoValor) throws Exception {
        if (novoValor < 0)
            throw new Exception("Valor de utilizacoes nao pode ser negativo.");
        ...
        try (Connection con = getConnection()) {
            ...
            if (!"COMPUTADOR".equalsIgnoreCase(tipo)) {// validar que é computador
            ...
            try (PreparedStatement ps = con.prepareStatement(
                ...
                ps.setInt(2, idRecurso);
                int n = ps.executeUpdate();
                if (n == 0)// se ninguem foi atualizado
                    throw new Exception("Computador nao encontrado na tabela especifica.");
            }
