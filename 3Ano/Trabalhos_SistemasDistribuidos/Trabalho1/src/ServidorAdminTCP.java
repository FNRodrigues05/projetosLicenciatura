...
public class ServidorAdminTCP implements Runnable {
    ...
    // handler para cada cliente admin
                    ...
                    try {
                        switch (cmd) {
                            ...
                            case "APROVAR": {
                                ...
                                String msg = gestorBD.aprovarRecurso(id);
                                out.println(msg);
                                break;
                            }
                            // Alterar estado recurso
                            case "SET_ESTADO": {
                                int id = Integer.parseInt(parts[1]);
                                String estado = parts[2].toUpperCase();
                                gestorBD.alterarEstadoRecurso(id, estado);
                                out.println("Estado do recurso " + id + " atualizado para " + estado + ".");
                                break;
                            }
                            // Alterar numero copias (livro)
                            case "SET_COPIAS": {
                                int id = Integer.parseInt(parts[1]);
                            ...
                            // Alterar utilizacoes (computador)
                            case "SET_UTILIZACOES": {
                                if (parts.length < 3) {
                                    ...
                                    gestorBD.alterarUtilizacoesComputador(id, novo);
                                    out.println("Computador #" + id + " agora tem utilizacoes = " + novo + ".");
                                }
                                break;
                            }
                            // Listar utilizadores aprovados
                            ...
                            // Listar utilizadores pendentes(Nao aprovados)
