    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (true) {
                ...
                double tempFinal = Math.round(currentTemp * 100.0) / 100.0;
                double humFinal = Math.round(currentHum * 100.0) / 100.0;

                System.out.printf(">>> [%s] A enviar: Temp %.2f | Hum %.2f%n", deviceId, tempFinal, humFinal);
                
                MetricRequest request = MetricRequest.newBuilder()
                        .setDeviceId(deviceId)
                        .setTemperature(tempFinal)
                        .setHumidity(humFinal)
