...
Este projeto foi desenvolvido no âmbito da disciplina de Sistemas Digitais da Universidade de Évora
...
* **Flip-Flops:** Utilizados para o controlo de estado das máquinas de Moore/Mealy.
...
Para detalhes profundos sobre a máquina de estados, consulte o documento `Relatório SD 58354-59100-59119.pdf` incluído neste repositório. O relatório contém:
...
1. Faça o download do [Logisim](http://www.cburch.com/logisim/), certificando-se de que possui o Java instalado.
2. Abra o Logisim.
3. Vá a `File -> Open` e selecione o ficheiro `Trabalho SD.circ` presente nesta pasta.
...
5. Para simular o funcionamento temporal:
   * Vá ao menu `Simulate` e certifique-se de que a opção **Simulation Enabled** (Habilitar Simulação) está ativa.
   * Ainda no menu `Simulate`, ative a opção **Ticks Enabled** (Pulso Habilitado) ou ajuste a *Tick Frequency* para ver a contagem do display e as transições de cor a acontecerem automaticamente no tempo.
6. Use a ferramenta do ícone da mão para alterar o estado do pino `bp || sv` (simulando a pressão do botão por um peão).